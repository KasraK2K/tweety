<x-app>
	<form action="{{ $user->path() }}" method="POST" enctype="multipart/form-data">
		...
		<div class="mb-6">
			...
			<input value="{{ $user->name }}" class="border border-gray-400 p-2 w-full" type="text" name="name" id="name" required>
			@error('name')
		...
		<div class="mb-6">
			...
			<input value="{{ $user->username }}" class="border border-gray-400 p-2 w-full" type="text" name="username" id="username" required>
			@error('username')
				<p class="text-red-500 text-xs mt-2">{{ $message }}</p>
			@enderror
		</div>
		...
		<div class="mb-6">
			...
			<input value="{{ $user->email }}" class="border border-gray-400 p-2 w-full" type="email" name="email" id="email" required>
			@error('email')
				<p class="text-red-500 text-xs mt-2">{{ $message }}</p>
			@enderror
		</div>
		...
		<div class="mb-6">
			<button type="submit" class="bg-blue-400 text-white rounded py-2 px-6 hover:bg-blue-500 mr-4">
				Submit
			</button>
